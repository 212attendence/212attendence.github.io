(function(window){
  ...
  function enforceOptionalNotificationConsent(){
    ...
    if(window.AttendanceApp&&typeof AttendanceApp.jsonp==="function"&&!AttendanceApp.__studentPrivacyJsonpWrapped){
      ...
      AttendanceApp.jsonp=function(url,options){var text=String(url||"");if((text.indexOf("action=pushClientConfigJsonp")>=0||text.indexOf("action=saveStudentPushTokenJsonp")>=0)&&!pushAllowed())return Promise.resolve({ok:false,code:"PUSH_CONSENT_REQUIRED",message:"알림 선택 동의가 필요합니다."});return originalJsonp(url,options)};
      AttendanceApp.__studentPrivacyJsonpWrapped=true;
    }
    if(window.AttendanceRoleApp&&!AttendanceRoleApp.__studentPrivacyWrapped){
      var originalRegister=AttendanceRoleApp.registerPushToken&&AttendanceRoleApp.registerPushToken.bind(AttendanceRoleApp),originalLocal=AttendanceRoleApp.showLocalNotification&&AttendanceRoleApp.showLocalNotification.bind(AttendanceRoleApp);
